...
function vaciarCarritoConTotal() {
    ...
    console.log(carrito, total.toFixed(1));
}
...
function cuentaDeAlumnos() {
    ...
    const arreglo = grupoA.concat(grupoB);
...
function transmitiendoBatch() {
    ...
    const tamañolista = backup.length / 4;

    for(let i =0; i<backup.length; i += tamañolista)
    {
        const nuevalista = backup.slice(i, i + tamañolista);
        console.log(nuevalista, nuevalista.length );
    }
}
